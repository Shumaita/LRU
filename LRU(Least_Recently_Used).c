...
// C implementation of LRU page replacement with the help of chatgpt
...
int main()
{
    ...
    //a[] is an array used to store the reference string,b[] is an array used to represent the frame or the physical memory
    ...
    for(j=0; j<m; j++)
        b[j]=-1; // Initialize all frame elements with -1 m=3

    // Loop Complexity: O(n*m)
    for(i=0; i<n; i++) // Loop through each reference string n=20
    {
        // Loop Complexity: O(m)
